...
##
def readArea(case):
	with codecs.open(case + 'inTable.csv', 'r', encoding='iso-8859-1') as f:
...
def readConsume(case):
	...
	consume = []
	csvFile = open(case + 'inTable.csv', 'r', encoding='iso-8859-1')
...
def readEnthalpies(case, onlyPartial):
	...
	foundEnthalpy = False
	enthalpies = []
	csvFile = open(case + 'in.csv', 'r', encoding='iso-8859-1')
	csvReader = csv.reader(csvFile, delimiter=',')
...
def writeChillers(case):
	...
	indexes = []
	newRow = []
	csvFile = open(case + 'in.csv', 'r')
	csvChillers = open(case + 'chillers.csv', 'w')
	csvReader = csv.reader(csvFile, delimiter=',')
...
def chillerFromJouleToKW(case, area):
	...
	total = 0.0
	newChillerValues = []
	csvFile = open(case + 'chillers.csv', 'r')
	csvReader = csv.reader(csvFile, delimiter=',')
...
def calculateHM(enthalpies):
	...
	hm = [enthalpy - 52 for enthalpy in newEnthalpies]
...
##
def calculateChe(enthalpies, hMax):
	enthalpies = [float(enthalpy)/1000 for enthalpy in enthalpies]
	hm = [enthalpy - 52 for enthalpy in enthalpies]
	hm = [h for h in hm if h > 0]
	sumHM = sum(hm)
...
def testFileAndWriteHeader():
	if not os.path.isfile('resultados.csv'):
		i = 0
		header = ['Case', 'City', 'State', 'Area']
		header += ['Heat rejection consume by m²', 'Cooling consume by m²']
		header += ['Pumps consume by m²', 'Fans by m²', 'Total consumption']
...
def run(case):
	...
	cheTotal = calculateChe(totalEnthalpies, hMax)
	chePartial = calculateChe(partialEnthalpies, hMax)
	...
	row += list(cityState) + [area] + consumeBySquareMeter + chillerValuesTons
